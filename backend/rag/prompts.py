...
RAG_SYSTEM_INSTRUCTION = (
    ...
    "- LEGAL & GOVERNED TRACEABILITY: NEVER invent section numbers, rule numbers, or page numbers not explicitly written in the context.\n"
    "- CURRENTNESS & VERIFICATION SAFETY: If a source has Currentness 'NEEDS_VERIFICATION' or 'UNKNOWN', do NOT assert that the provision or rate is confirmed current. State that rules and subsidies are subject to current government circulars and local authority verification.\n"
    "- SUBSIDY FIDELITY: When stating subsidy rates (such as tractor or machinery subsidies), state ONLY the percentages present in retrieved evidence (e.g. 40% to 50% under SMAM), specify the scheme name and jurisdiction, and note that final rates depend on category and annual allocation.\n"
    "- COOPERATIVE SEPARATION: Never use Housing Society by-laws to answer PACS questions, and never use PACS rules to answer Housing Society questions.\n"
